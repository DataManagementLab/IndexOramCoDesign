...
static sgx_status_t SGX_CDECL Enclave_ocall_get_oram_batch(void* pms)
{
	ms_ocall_get_oram_batch_t* ms = SGX_CAST(ms_ocall_get_oram_batch_t*, pms);
	ms->ms_retval = ocall_get_oram_batch(ms->ms_instance, ms->ms_leaves, ms->ms_leaves_len);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write_oram_batch(void* pms)
{
	ms_ocall_write_oram_batch_t* ms = SGX_CAST(ms_ocall_write_oram_batch_t*, pms);
	ms->ms_retval = ocall_write_oram_batch(ms->ms_instance, ms->ms_leaves, ms->ms_leaves_len, ms->ms_buckets, ms->ms_buckets_len);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_generic_request(void* pms)
{
	ms_ocall_generic_request_t* ms = SGX_CAST(ms_ocall_generic_request_t*, pms);
	ms->ms_retval = ocall_generic_request(ms->ms_request, ms->ms_request_len);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_setup_oram(void* pms)
{
	ms_ocall_setup_oram_t* ms = SGX_CAST(ms_ocall_setup_oram_t*, pms);
	ms->ms_retval = ocall_setup_oram(ms->ms_instance, ms->ms_offset, ms->ms_buckets, ms->ms_buckets_len);
...
static const struct {
	...
	65,
	{
		(void*)Enclave_ocall_get_oram_batch,
		(void*)Enclave_ocall_write_oram_batch,
		(void*)Enclave_ocall_generic_request,
		(void*)Enclave_ocall_setup_oram,
...
sgx_status_t ecall_generic_request(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* request, uint32_t request_len)
{
	sgx_status_t status;
	ms_ecall_generic_request_t ms;
...
sgx_status_t ecall_run_tests(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
...
sgx_status_t ecall_return_oram_path(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* buckets, uint32_t buckets_len, uint64_t single_bucket_len)
{
	sgx_status_t status;
	ms_ecall_return_oram_path_t ms;
	ms.ms_buckets = buckets;
	ms.ms_buckets_len = buckets_len;
	ms.ms_single_bucket_len = single_bucket_len;
